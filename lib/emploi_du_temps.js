...
EmploiDuTemps.prototype.calculerVolumeHoraire = function () {
    var volumeHoraire = 0;
    for (jours of this.pdt) {
        for (heure of this.pdt[jours]) {
            if (this.pdt[jours][heures] == true) {
                volumeHoraire += 0.5;
            }
        }
    }
...
EmploiDuTemps.prototype.calculerComplementaire = function () {
    ...
    for (jours of this.pdt) {
        for (heure of this.pdt[jours]) {
            edt2.pdt[jours][heures] = this.pdt[jours][heures] != true;
        }
    }
    return edt2;
};
...
EmploiDuTemps.prototype.exportCsv = function () {
    ...
    fs.writeFile('./' + intervenant+'_edt.csv', '', function(err) {
    if(err) {
        ...
        for(var i =0 ; i<48 ; i++)
        {
            for(var j = 0; j < 7; j++)
            {
                ...
                fs.appendFileSync('./' + intervenant+'_edt.csv', data[i][j]+';', encoding='utf8'); 

            }
            fs.appendFileSync('./' + intervenant+'_edt.csv', '\r\n', encoding='utf8'); 
...
EmploiDuTemps.prototype.exportiCal = function () {
    ...
    var intervenant  = this.intervenant;
    fs.open('./' + intervenant+'_edt.ics','w', function(err) {
    if(err) {
        return console.log(err);
    }
        fs.writeFile('./' + intervenant+'_edt.ics', iCal, function(err) {
        if(err) {
            return console.log(err);
        }
