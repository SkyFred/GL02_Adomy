...
function ajout_pivot(start, end)
{
	//On récupère le jour de début
	var jDebut = parseInt(start[6])*10 +parseInt(start[7]);
	//On récupère l'heure de début
	var hDebut = parseFloat(start[9])*10+parseFloat(start[10]);
	//On récupère les minutes du début
	var mDebut = parseFloat(start[11])*10+parseFloat(start[12]);
	//De même pour la fin
	var jFin = parseInt(start[6])*10 +parseInt(start[7]);
	var hFin = parseFloat(end[9])*10+parseFloat(end[10]);
	var mFin = parseFloat(end[11])*10+parseFloat(end[12]);
	...
	if ((jFin - jDebut) > 7)
		console.log("Emploi du temps de 7 jours...");
	else
		ajouter(pdt, (24-(24-hFin)+24-(24-hDebut) + jFin - jDebut), jDebut, hDebut);
}
